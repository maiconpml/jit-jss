...
int main(int argc, char *argv[]) {
	...
	for(unsigned x=3; x<=10; x++) {
		...
		for(unsigned maxP = 10; maxP<=1000; maxP*=10) {
			...
			for(unsigned rep=0; rep<10; rep++) { 
				fileName = "./GholamiExtraInsts/GhSo_"+unsigStr(index)+"_rep_"+unsigStr(rep)+"_jm_"+unsigStr(x)+"_maxP_"+unsigStr(maxP-1)+".ppsi";
	...
	for(unsigned x=10; x<=160; x+=10) {
		...
		for(unsigned rep=0; rep<10; rep++) {
			fileName = "./GholamiExtraInsts/GhSo_"+unsigStr(index)+"_rep_"+unsigStr(rep)+".ppsi"+"_jm_"+unsigStr(x)+"_maxP_"+unsigStr(maxP)+".ppsi";
